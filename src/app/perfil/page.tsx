...
import { UserCircle, ShieldCheck, Key, AlertTriangle, Mail, Lock } from 'lucide-react';
    ...
    <div className="flex h-screen bg-muted/30">
      ...
      <main className="flex-1 overflow-auto p-8">
        <div className="max-w-3xl mx-auto space-y-8">
          <div><h1 className="text-3xl font-black uppercase text-primary">Meu Perfil</h1><p className="text-[10px] font-black uppercase opacity-60">Gestão de Conta e Acesso</p></div>
          
          <form onSubmit={handleUpdate} className="space-y-6">
            <Card className="border-t-4 border-t-accent shadow-xl rounded-2xl">
              <CardHeader><CardTitle className="text-sm font-black uppercase flex items-center gap-2"><UserCircle className="w-4 h-4 text-accent" /> Dados Pessoais</CardTitle></CardHeader>
            ...
            <Card className="border-t-4 border-t-primary shadow-xl rounded-2xl">
              <CardHeader><CardTitle className="text-sm font-black uppercase flex items-center gap-2"><Mail className="w-4 h-4 text-primary" /> Credenciais de Acesso</CardTitle></CardHeader>
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-black uppercase">Usuário / Email</Label>
                    <Input value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="font-bold h-11" />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-black uppercase">Senha</Label>
                    <Input type="text" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="font-bold h-11" />
                  </div>
                </div>
              </CardContent>
            </Card>
