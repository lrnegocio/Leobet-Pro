  ...
  const handleSearch = async (searchCode?: string) => {
    ...
    try {
      ...
      if (found) {
        ...
        // Busca todos os tickets e vendedores para calcular prêmio dinâmico real
            ...
            <div className="space-y-6">
              <div className="bg-primary p-6 rounded-[2rem] text-white flex justify-between items-center shadow-lg">
                 <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-[10px] font-black uppercase opacity-60">Prêmio Acumulado (Líquido)</p><p className="text-3xl font-black">R$ {eventPool.toFixed(2)}</p></div>
                 </div>
                 <Badge className="bg-accent text-white font-black uppercase text-[9px]">Auditado</Badge>
              </div>

              {statsGanhos.total > 0 && (
                <div className="p-8 rounded-[2rem] text-white flex justify-between items-center shadow-xl bg-green-600">
              ...
              <div className="bg-primary/5 p-8 rounded-[2rem] border-2 border-primary/10">
                ...
                {eventData && (
                  <div className="mb-6 p-4 bg-white border-2 border-dashed rounded-2xl">
                    <p className="text-[9px] font-black uppercase text-primary flex items-center gap-1 mb-2"><Info className="w-3 h-3" /> Regras:</p>
                    <p className="text-[10px] font-bold text-muted-foreground whitespace-pre-line">{eventData.regras || 'Rateio automático auditado.'}</p>
                  </div>
                )}
              </div>
            </div>
