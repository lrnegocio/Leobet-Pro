              ...
              <Card key={u.id} className="border-l-8 border-l-primary rounded-3xl overflow-hidden bg-white">
                <CardContent className="p-6 flex flex-col md:flex-row justify-between items-center gap-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button onClick={async () => { if(confirm("EXCLUIR?")) { await supabase.from('users').delete().eq('id', u.id); loadUsers(); } }} variant="ghost" size="icon" className="h-12 w-12 text-destructive border rounded-xl"><Trash2 className="w-5 h-5" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))}
