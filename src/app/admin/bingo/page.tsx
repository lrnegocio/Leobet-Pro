  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: err.message || "Verifique a conexão ou as chaves no Vercel."
      });
    } finally {
      setLoading(false);
      setSyncing(false);
      ...
      <main className="flex-1 overflow-auto p-4 md:p-8 pt-20 lg:pt-8">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 px-4 md:px-0">
            {loading ? (
              <div className="py-20 text-center animate-pulse font-black uppercase text-muted-foreground text-xs">Acessando Banco de Dados...</div>
            ) : bingos.length === 0 ? (
                    ...
                    <div className="flex flex-col md:flex-row items-stretch">
                       <div className="p-6 flex-1 space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <Badge variant={isFinished ? 'secondary' : (isSalesClosed ? 'destructive' : 'default')} className="font-black text-[9px] uppercase h-6 px-3">
                       ...
                       <div className="bg-muted/50 p-6 flex items-center gap-3 border-l shrink-0">
                          <div className="flex flex-col gap-2 min-w-[140px] w-full md:w-auto">
                             ...
                             <Link href={isFinished ? `/admin/financeiro` : `/admin/bingo/sorteio/${bingo.id}`} className="w-full">
                               <Button 
                                 className={cn(
                                   "w-full gap-2 font-black uppercase text-xs h-12 shadow-xl",
