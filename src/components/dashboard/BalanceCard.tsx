...
export function BalanceCard() {
  ...
  const fetchMasterPix = async () => {
    try {
      ...
      if (data && data.length > 0 && data[0].pix_key) {
  ...
  const handleDepositRequest = async () => {
    ...
    try {
      await supabase.from('transactions').insert([{
        user_id: user.id, user_name: user.nome, amount: amount, type: 'deposito', status: 'pendente'
      }]);
      const msg = `*DEPÓSITO - LEOBET*%0A👤 *CLIENTE:* ${user.nome}%0A💰 *VALOR:* R$ ${amount.toFixed(2)}%0A*PIX:* ${masterPix}`;
      window.open(`https://api.whatsapp.com/send?phone=5582993343941&text=${msg}`, '_blank');
      setOpenDeposit(false); setDepositAmount('');
      toast({ title: "SOLICITAÇÃO ENVIADA!" });
    } catch (err: any) { toast({ variant: "destructive", title: "FALHA" }); }
        ...
        {user.role === 'admin' ? (
          ...
          <div className="flex gap-3">
            <Dialog open={openDeposit} onOpenChange={setOpenDeposit}>
              <DialogTrigger asChild><Button variant="outline" onClick={fetchMasterPix} className="flex-1 border-white/20 hover:bg-white/10 text-white gap-2 uppercase font-black text-[10px] h-14 rounded-2xl"><ArrowUpCircle className="w-4 h-4" /> Depósito</Button></DialogTrigger>
              <DialogContent className="bg-white rounded-[2.5rem]">
                <DialogHeader><DialogTitle className="font-black uppercase text-primary text-center">Recarregar via PIX</DialogTitle></DialogHeader>
                <div className="space-y-6 py-4">
                   <div className="bg-primary/5 p-6 rounded-3xl border-2 border-primary/10 space-y-4">
                      <p className="text-[10px] font-black uppercase text-muted-foreground text-center">Chave PIX Oficial</p>
                      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border-2 border-primary/20">
                        ...
                        <Button onClick={copyPix} size="icon" variant="ghost" className="text-primary"><Copy className="w-5 h-5" /></Button>
                      </div>
                   </div>
                ...
                <DialogFooter><Button onClick={handleDepositRequest} disabled={loading} className="w-full h-16 font-black bg-primary rounded-2xl text-white">ENVIAR COMPROVANTE</Button></DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
        )}
